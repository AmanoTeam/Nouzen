...
typedef enum CIKeyType cir_key_type_t;
...
typedef enum CIKeySubype cir_key_subtype_t;

struct CIKey {
	cir_key_type_t type;
	cir_key_subtype_t subtype;
	char* name;
#if defined(_WIN32)
	WORD code[5];
#else
...
typedef struct CIKey cir_key_t;

static const cir_key_t KEYBOARD_KEY_EMPTY = {
	KEY_EMPTY,
	KEY_SUBTYPE_UNKNOWN,
	"Empty",
	#if defined(_WIN32)
...
static const cir_key_t KEYBOARD_KEY_UNKNOWN = {
	KEY_UNKNOWN,
	KEY_SUBTYPE_UNKNOWN,
	"Unknown",
	#if defined(_WIN32)
